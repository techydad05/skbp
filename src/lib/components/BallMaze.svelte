<script>
	...
	function restartGame() {
		...
		ball = Bodies.circle(width * 0.2, height * 0.2, ballRadius, {
			restitution: 0.5,
			friction: 0.1,
			density: 0.002,
				...
				DeviceOrientationEvent.requestPermission()
					.then((response) => {
						if (response === 'granted') {
							...
							toastMessage = 'Motion controls enabled!';
							showToast = true;
							setTimeout(() => (showToast = false), 3000);
						} else {
							toastMessage = 'Motion controls denied. Using touch controls instead.';
							showToast = true;
							setTimeout(() => (showToast = false), 3000);
						}
					})
					.catch((error) => {
						toastMessage = 'Error accessing motion controls. Using touch controls.';
						showToast = true;
						setTimeout(() => (showToast = false), 3000);
					});
			} else {
